...
def run(query, constraint):
    ...
    #
    plt.xticks(np.arange(0, 8), x_list, rotation=0, fontsize=70)
    plt.yticks(fontsize=70, weight='bold')

    plt.xlabel(r'num-children, 1 apart', fontsize=70, weight='bold')
    # plt.ylabel('Running time (s)')
    plt.legend(loc='upper right', bbox_to_anchor=(1.03, 1.0), fontsize=60)
    plt.tight_layout()
