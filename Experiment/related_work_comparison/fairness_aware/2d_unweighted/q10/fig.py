...
input_path = r'competitior_q10.csv'
...
input_path = r'constraint_change_q10.csv'
input_file = open(input_path, "r")
...
plt.xlabel('target disparity: \% of the original disparity', fontsize=65, weight='bold').set_position((0.44, 0))
# plt.ylabel('Running time (s)')
plt.yscale('log')
# plt.legend(loc='upper right', bbox_to_anchor=(1, 1.05), ncol=2, fontsize=40)
lgnd = plt.legend(loc='upper center', bbox_to_anchor=(0.46, 1.35), fontsize=50, ncol=4, labelspacing=0.2,
                  handletextpad=0.2, markerscale=0.2, columnspacing=0.2, frameon=False)

plt.tight_layout()
plt.savefig("comparision_q10.png", bbox_inches='tight')
plt.show()
