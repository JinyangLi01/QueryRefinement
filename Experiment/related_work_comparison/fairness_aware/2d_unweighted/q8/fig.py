...
label = ['PVL-total', "PVL-avg", "PVL-first", "MP",]
...
input_path = r'competitior_q8.csv'
...
input_path = r'constraint_change_q8.csv'
input_file = open(input_path, "r")
...
plt.xticks(x_pos + 0.5 * bar_width, [80, 70, 60, 50, 40, 30, 20], fontsize=65, weight='bold')
plt.yticks(fontsize=65, weight='bold')

plt.xlabel('target disparity = \% of the original disparity', fontsize=65, weight='bold')
# plt.ylabel('Running time (s)')
plt.yscale('log')
# plt.legend(loc='upper right', bbox_to_anchor=(1, 1.05), ncol=2, fontsize=40)
lgnd = plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.3), fontsize=50, ncol=4, labelspacing=0.3,
                  handletextpad=0.4, markerscale=0.5, columnspacing=0.4, frameon=False)

plt.tight_layout()
plt.savefig("comparision_q8.png", bbox_inches='tight')
plt.show()
