...
support predicates on same attributes: x >= 5, x <= 20
copied from ProvenanceSearchValues_8_complex_optimized.py
add optimizations for single binary sensitive attribute situation
"""
...
def build_PVT_refinement(data, selected_attributes, numeric_attributes,
    ...
    def itercol(col):
        ...
        if att in selection_numeric:
            if len(selection_numeric[att]) == 2:
                def add_to_PVT(strr):
                    col_in_pvl = att + strr
    ...
    data = data.drop_duplicates(subset=selected_attributes, keep='first').reset_index(drop=True)
    possible_values_sets = dict()
    ...
    for att in PVT_head:
        ...
        elif att.split('__')[0] in selection_numeric:
            if att.split('__')[1] == 'greater':
                possible_values_lists[att].sort(
                    key=lambda p: abs(p - selection_numeric[att.split('__')[0]]['greater'][1]))
            else:
                possible_values_lists[att].sort(
                    key=lambda p: abs(p - selection_numeric[att.split('__')[0]]['smaller'][1]))
        else:
            pre, v = att.split("__")
    ...
    # TODO: here lists are sorted by lengths, so x__greater is not necessarily always before x__smaller, need to fix somewhere later
        ...
        for va in numeric_attributes:
            if va in value_assignment:
                if selection_numeric[va][0] == '>':
                    ...
                    pe_dataframe = pe_dataframe[pe_dataframe[va] <= value_assignment[va]]
            elif va + '__greater' in value_assignment or va + '__smaller' in value_assignment:
                if selection_numeric[va]["greater"][0] == '>':
                    pe_dataframe = pe_dataframe[pe_dataframe[va] > value_assignment[va + '__greater']]
                if selection_numeric[va]["greater"][0] == ">=":
                    pe_dataframe = pe_dataframe[pe_dataframe[va] >= value_assignment[va + '__greater']]
                if selection_numeric[va]["smaller"][0] == "<":
                    pe_dataframe = pe_dataframe[pe_dataframe[va] < value_assignment[va + '__smaller']]
                if selection_numeric[va]["smaller"][0] == "<=":
                    pe_dataframe = pe_dataframe[pe_dataframe[va] <= value_assignment[va + '__smaller']]
        included = [x for x in value_assignment if (x not in numeric_attributes and value_assignment[x] == 1)]
        new_select = copy.deepcopy(selection_categorical)
    ...
    if len(left_PVT_list) - left_idx_PVT < right_idx_PVT:
        ...
        for j in range(0, right_idx_PVT):
            ...
            if left_PVT_idx_bound == left_idx_PVT + 1:
                break
    if len(result) > 0:
        logging.debug("two column, find other result = {}".format(result))
...
def same_att_first_two_columns(data, two_col_PVT, two_col_PVT_head, full_value_assignment,
    ...
    cur_lowerbound = full_value_assignment[two_col_PVT_head[0]]
    cur_upperbound = full_value_assignment[two_col_PVT_head[1]]
    fixed_att_idx_PVT = [tight_value_idx, original_idx - 1]
    other_att_idx_PVT = [0, full_value_assignment_positions[two_col_PVT_head[0]] - 1]
    if fixed_att.split('__')[1] == "smaller":
        right_att_idx_PVT = fixed_att_idx_PVT
        left_att_idx_PVT = other_att_idx_PVT
        right_col_name = fixed_att
        left_col_name = fixed_att.split('__')[0] + "__smaller"
    else:
        left_att_idx_PVT = fixed_att_idx_PVT
        right_att_idx_PVT = other_att_idx_PVT
        left_col_name = fixed_att
        right_col_name = fixed_att.split('__')[0] + "__smaller"
    disparity_goal = fairness_constraints_provenance_complex[0]['plus']
    fixed_values = copy.deepcopy(full_value_assignment)
    del fixed_values[two_col_PVT_head[0]]
    del fixed_values[two_col_PVT_head[1]]
    temp_data = copy.deepcopy(data)
    # apply fixed columns
    for k, v in fixed_values.items():
        att = k.split('__')[0]
        if k == att + "__smaller":
            temp_data = temp_data[temp_data[att] <= v]
        else:
            temp_data = temp_data[temp_data[att] >= v]
    att = two_col_PVT_head[0].split('__')[0]
    temp_data = temp_data.sort_values(by=att)
    value_list = temp_data[att].tolist()
    cur_left_idx_valuelist = bisect(value_list, cur_lowerbound) - 1
    cur_right_idx_valuelist = bisect(value_list, cur_upperbound) - 1
    # idx in value list
    left_PVT_list = two_col_PVT[left_col_name].tolist()[left_att_idx_PVT[0] : left_att_idx_PVT[1] + 1]
    right_PVT_list = two_col_PVT[right_col_name].tolist()[right_att_idx_PVT[0] : right_att_idx_PVT[1] + 1]
    s = temp_data[cur_left_idx_valuelist : cur_right_idx_valuelist + 1]
    cur_blue_num = len(s[s[binary_sensitive_att] == 1])
    cur_red_num = cur_right_idx_valuelist + 1 - cur_left_idx_valuelist - cur_blue_num
    diff = cur_blue_num - cur_red_num
    print(cur_blue_num, cur_red_num)
    result = []
    right_upper_idx = right_att_idx_PVT[0]
    if left_att_idx_PVT[0] == 0:
        for i in range(0, left_att_idx_PVT[1]):
            lb = left_PVT_list[i]
            for j in range(right_att_idx_PVT[0], right_upper_idx):
                ub = right_PVT_list[j]
                s = temp_data[(temp_data[att] >= lb) & (temp_data[att] <= ub)]
                blue_num = len(s[s[binary_sensitive_att] == 1])
                red_num = len(s[s[binary_sensitive_att] == 0])
...
def searchPVT_refinement(data, PVT, PVT_head, possible_values_lists, numeric_attributes, categorical_attributes,
                         ...
                         checked_assignments_satisfying, checked_assignments_not_satisfying,
                         single_binary_sensitive_att, first_minimal_refinement = True,
                         time_limit=5 * 60):
    time1 = time.time()
    global assign_to_provenance_num
    assign_to_provenance_num = 0
    ...
    while PVT_stack:
        ...
        logging.debug("fixed_value_assignments_positions: {}".format(fixed_value_assignments_positions))
        ...
        # logging.debug("to_put_to_stack: {}".format(to_put_to_stack))
        logging.debug("num in to_put_to_stack: {}".format(len(to_put_to_stack)))
        logging.debug("len of stack: {}".format(len(PVT_stack)))
        if fixed_value_assignments_positions == {'y__smaller': 0, 'y__greater': 11}:
            logging.debug("debug, stop here")
        new_value_assignment = {}
        full_value_assignment = {}
        last_satisfying_bounding_relaxation_location = []
        ...
        while att_idx < num_columns and att_idx >= 0:
            ...
            # binary search in the last column numeric, new_value_assignment_position[att_idx] must be 0
            if att_idx == num_columns - 1 and (
                    col in numeric_attributes or col.split('__')[0] in numeric_attributes):  # last column
                original = new_value_assignment_position[att_idx]
                ...
                assign, originalreason = assign_to_provenance(full_value_assignment, numeric_attributes,
                                                              categorical_attributes, selection_numeric,
                                                              selection_categorical, full_PVT_head,
                                                              fairness_constraints_provenance_greater_than,
                                                              fairness_constraints_provenance_smaller_than,
                ...
                if assign:
                    # checked_assignments_satisfying.append(full_value_assignment_str)
                    # logging.info("{} satisfies constraints".format(full_value_assignment))
                    ...
                    idx_in_col = original
                    break
                # logging.info("{} doesn't satisfy constraints due to {}".format(full_value_assignment, originalreason))

                right = len(values_to_consider) - 1
                left = original + 1
                idx_list = range(left, right + 1)
                if originalreason == 3:  # traversal, can't do binary search
                    if single_binary_sensitive_att:  # when only one binary sensitive attribute
                        temp_data = copy.deepcopy(data)
                        # apply fixed value assignments
                        for k, v in fixed_value_assignments.items():
                            att = k.split('__')[0]
                            if k == att + "__smaller":
                                temp_data = temp_data[temp_data[att] <= v]
                                ...
                                temp_data = temp_data[temp_data[att] >= v]
                        for i in range(0, att_idx):
                            col_name = PVT_head[i].split('__')[0]
                            if PVT_head[i] == col_name + "__smaller":
                                temp_data = temp_data[temp_data[col_name] <= full_value_assignment[PVT_head[i]]]
                            else:
                                temp_data = temp_data[temp_data[col_name] >= full_value_assignment[PVT_head[i]]]
                        temp_data = temp_data.sort_values(by=col.split('__')[0],
                                                          key=lambda x: abs(x - values_to_consider[original]))
                        # logging.info(temp_data)
                        binary_sensitive_att = 'blue'
                        disparity_goal = fairness_constraints_provenance_complex[0]['plus']
                        res_idx = -1
                        res_val = -1
                        for i, (y_value, _) in enumerate(
                                temp_data[[col.split('__')[0], binary_sensitive_att]].itertuples(index=False), start=0):
                            if col.split('__')[1] == "smaller":
                                subset = temp_data[temp_data[col.split('__')[0]] <= y_value]
                            else:
                                subset = temp_data[temp_data[col.split('__')[0]] >= y_value]
                            blue_0_count = subset[subset[binary_sensitive_att] == 0].shape[0]
                            blue_1_count = subset[subset[binary_sensitive_att] == 1].shape[0]
                            if abs(blue_0_count - blue_1_count) <= disparity_goal:
                                res_idx = i
                                res_val = y_value
                                break
                        if res_idx > -1:
                            if (res_val <= PVT.iloc[0, att_idx] and col.split('__')[1] == "smaller") or \
                                ...
                                # logging.info("{} satisfies constraints, id = {}".format(full_value_assignment, res_idx))
                            else:
                        ...
                        # only one binary sensitive attribute end
                    else:
                        while left <= right:
                            # logging.debug("left = {}, right = {}".format(left, right))
                            new_value_assignment[col] = values_to_consider[left]
                            full_value_assignment = {**new_value_assignment, **fixed_value_assignments}
                            # full_value_assignment_str = num2string(
                            #     [full_value_assignment[k] for k in full_PVT_head if k in full_att])
                            assign, thisreason = assign_to_provenance(full_value_assignment, numeric_attributes,
                                                                      categorical_attributes, selection_numeric,
                            ...
                            if assign:
                                ...
                                # logging.info("{} satisfies constraints, id = {}".format(full_value_assignment, idx_in_PVT))
                                new_value_assignment_position[att_idx] = idx_in_PVT
                                last_satisfying_bounding_relaxation_location = new_value_assignment_position
                                find_base_refinement = True
                                find_value_this_col = True
                                ...
                                # logging.info("{} doesn't satisfy constraints, id = {}".format(full_value_assignment, left))
                                left += 1
                    ...
                    if originalreason == 0 and (selection_numeric[col][0] == '>' or selection_numeric[col][0] == '>='):
                        idx_list = [x for x in idx_list if values_to_consider[x]
                    ...
                    # left reason
                    leftreason = originalreason
                    # right reason
                    new_value_assignment[col] = values_to_consider[idx_list[right]]
                    full_value_assignment = {**new_value_assignment, **fixed_value_assignments}
                    # full_value_assignment_str = num2string(
                    #     [full_value_assignment[k] for k in full_PVT_head if k in full_att])
                    ...
                    if rightreason == leftreason:
                        ...
                        att_idx -= 1
                        same_last_col_reason = True
                        lastreason = leftreason
                        continue
                    same_last_col_reason = False
                    while left <= right:
                        mid = int((left + right) / 2)
                        # logging.info(left, mid, right)
                        new_value_assignment[col] = values_to_consider[idx_list[mid]]
                        full_value_assignment = {**new_value_assignment, **fixed_value_assignments}
                        # full_value_assignment_str = num2string(
                        #     [full_value_assignment[k] for k in full_PVT_head if k in full_att])
                        assign, thisreason = assign_to_provenance(full_value_assignment, numeric_attributes,
                        ...
                        if assign:
                            ...
                            if thisreason == leftreason:
                                ...
                                right = mid - 1
                    if find_base_refinement:
                        new_value_assignment[col] = values_to_consider[new_value_assignment_position[att_idx]]
                        full_value_assignment[col] = values_to_consider[new_value_assignment_position[att_idx]]
                        break
                    else:
                        new_value_assignment_position[att_idx] = -1
                        del new_value_assignment[col]
                        att_idx -= 1
                        continue
            elif same_last_col_reason:
                find_it = False
                if col not in numeric_attributes:  # categorical attributes
                    if new_value_assignment_position[att_idx] < 2 and lastreason + possible_values_lists[col][1] == 1:
                        new_value_assignment[col] = possible_values_lists[col][1]
                        new_value_assignment_position[att_idx] = 1
                        att_idx += 1
                        same_last_col_reason = False
                        continue
                ...
                for idx_in_col in range(new_value_assignment_position[att_idx], max_index_PVT[att_idx] + 1):
                    if idx_in_col not in idx_to_consider:
                        continue
                    value = possible_values_lists[col][idx_in_col]
                    if lastreason == 0 and (selection_numeric[col][0] == '>' or selection_numeric[col][0] == '>='):
                        if value < selection_numeric[col][1]:
                            find_it = True
                            break
                    elif lastreason == 0 and (selection_numeric[col][0] == '<' or selection_numeric[col][0] == '<='):
                        if value > selection_numeric[col][1]:
                            find_it = True
                            break
                    elif lastreason == 1 and (selection_numeric[col][0] == '>' or selection_numeric[col][0] == '>='):
                        if value > selection_numeric[col][1]:
                            find_it = True
                            break
                    else:
        ...
        if not find_base_refinement:
            ...
            check_to_put_to_stack_refinement(to_put_to_stack, next_col_num_in_stack, num_columns, find_relaxation,
                                             ...
                                             fixed_value_assignments_positions)
            # logging.info("no base refinement here, size of PVT: {}*{}".format(len(PVT), len(PVT_head)))
            search_space += len(PVT) * len(PVT_head)
            continue

        logging.debug("find base refinement {}".format(new_value_assignment))
        logging.debug("position: {}".format(new_value_assignment_position))
        tight_success = False
        tight_value_idx = -1
        fixed_att = str()
        ...
        if idx_in_this_col_in_parent_PVT > 0:
            ...
            if len(fairness_constraints_provenance_complex) > 0:  # can't use binary
                ...
                logging.debug("tighten the last fixed column {}:\n {}".format(fixed_att, values_above))
            ...
            if tight_value_idx >= 0:  # can be tightened
                if tight_value_idx < idx_in_this_col_in_parent_PVT:
                    # tight this fixed column successfully
                    logging.debug("after tighten last fixed col: {}".format(tight_value_idx))
                    fixed_value_assignments_for_tighten[fixed_att] = values_above[tight_value_idx]
                    full_value_assignment[fixed_att] = values_above[tight_value_idx]
                    tight_success = True
                fva = [full_value_assignment[k] for k in full_PVT_head]
            ...
            if tight_success and added:
                ...
                if len(to_put_to_stack) > 0:
                    if not all(v == 0 for v in last_satisfying_bounding_relaxation_location):
                        ...
                        # # only add values that are not filtered out
                        # # apply fixed columns
                        # temp_data = copy.deepcopy(data)
                        # for k, v in fixed_value_assignments.items():
                        #     if k == fixed_att:
                        ...
                        #     else:
                        #         temp_data = temp_data[temp_data[fixedatt] >= v]
                        # temp_data = temp_data.sort_values(by=fixed_att.split('__')[0])
                        # for j in range(idx_in_this_col_in_parent_PVT - 1, tight_value_idx, -1):
                        #     if values_above[j] in temp_data[fixed_att.split('__')[0]].values:
                        #         to_put = copy.deepcopy(to_put_to_stack[-1])
                        #         to_put['idx_in_this_col_in_parent_PVT'] = j
                        #         to_put['fixed_value_assignments'][fixed_att] = values_above[j]
                        #         to_put['fixed_value_assignments_to_tighten'] = values_above[:j]
                    ...
                    if tight_value_idx == 0 and len(to_put_to_stack) > 0:
                        ...
                        if len(to_put_to_stack) > 0:
                            to_put_to_stack[-1]['idx_in_this_col_in_parent_PVT'] = tight_value_idx - 1
                            to_put_to_stack[-1]['fixed_value_assignments'][fixed_att] = values_above[
                                tight_value_idx - 1]
                            to_put_to_stack[-1]['fixed_value_assignments_to_tighten'] = values_above[
                                                                                        : tight_value_idx - 1]
                            to_put_to_stack[-1]['fixed_value_assignments_positions'][fixed_att] = tight_value_idx - 1
        else:
            fva = [full_value_assignment[k] for k in full_PVT_head]
...
def FindMinimalRefinement(data_file_prefix, separator, query_file, constraint_file, data_file_format,
    ...
    if time.time() - time1 > time_limit:
        print("time out")
        return {}, [], time.time() - time1, assign_to_provenance_num, provenance_time, 0, 0
    original_PVT_head = copy.deepcopy(PVT_head)
    checked_assignments_satisfying = []
    checked_assignments_unsatisfying = []
    first_minimal_refinement = True
    minimal_refinements, time_first_minimal_refinement = searchPVT_refinement(data, PVT, PVT_head, possible_values_lists, numeric_attributes,
                                               categorical_attributes, selection_numeric_attributes,
                                               selection_categorical_attributes, len(PVT_head),
                                               fairness_constraints_provenance_greater_than,
                                               fairness_constraints_provenance_smaller_than,
                                               ...
                                               checked_assignments_satisfying,
                                               checked_assignments_unsatisfying, single_binary_sensitive_att,
                                               first_minimal_refinement,
                                               time_limit)
    time2 = time.time()
    print("provenance time = {}".format(provenance_time))
    print("searching time = {}".format(time2 - time_search1))
    print("assign_to_provenance_num = {}".format(assign_to_provenance_num))
    print("time to find the first minimal refinement = {}".format(time_first_minimal_refinement - time1))
    ...
    return reordered_minimal_refinements, order_in_results, time2 - time1, assign_to_provenance_num, \
        provenance_time, time2 - time_search1, time_first_minimal_refinement - time1
