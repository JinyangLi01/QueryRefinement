...
def whether_satisfy_fairness_constraints(data, selected_attributes, sensitive_attributes, fairness_constraints,
    ...
    for att in selection_categorical_attributes:
        num_cate += 1
        if len(selection_categorical_attributes[att]) == 0:
            return False
    ...
    # print(data_selected['num-children'].isnull().sum())
    # print(data_selected['income'].isnull().sum())
    # print(data_selected['county'].isnull().sum())
    # print(data_selected['id'].tolist())
    # print(data_selected[data_selected['id']==169])
    #
    # def select(row):
    #     for att in selection_numeric_attributes:
    ...
    #     return 1
    #
    # data_selected['satisfy_selection'] = data_selected[selected_attributes].apply(select, axis=1)
    # data_selected2 = data_selected[data_selected['satisfy_selection'] == 0]
    # print(len(data_selected2))
...
def LatticeTraversal(data, selected_attributes, sensitive_attributes, fairness_constraints,
                     numeric_attributes, categorical_attributes, selection_numeric_attributes,
                     selection_categorical_attributes, sql_file_read,
    ...
    if only_greater_than:
        return LatticeTraversalGreaterThan(data, selected_attributes, sensitive_attributes, fairness_constraints,
                                           numeric_attributes, categorical_attributes, selection_numeric_attributes,
                                           selection_categorical_attributes, sql_file_read,
                                           sql_connection, answer_columns, time_limit)
        ...
        if att_idx == num_numeric_att + num_cate_variables_to_add + num_cate_variables_to_remove:
            if legal:
                ...
                if whether_satisfy_fairness_constraints(data, selected_attributes, sensitive_attributes,
                                                        ...
                                                        new_selection_categorical_attributes, sql_file_read,
                                                        sql_connection,
                                                        answer_columns):
    ...
    while att_idx >= 0:
        ...
        if att_idx == num_numeric_att + num_cate_variables:
            if whether_satisfy_fairness_constraints(data, selected_attributes, sensitive_attributes,
                                                    ...
                                                    new_selection_numeric_attributes,
                                                    new_selection_categorical_attributes, sql_file_read, sql_connection,
                                                    answer_columns):
...
def FindMinimalRefinement(data_file, query_file, constraint_file, sql_file_read,
                          sql_connection, answer_column_file, time_limit=5 * 60):
    time1 = time.time()
