...
So this script is buggy

In ProvenanceSearch7, I would have a better way:
...
def update_stop_line(combo_w_t, stop_line, minimal_added_relaxations, sorted_table, delta_table,
    ...
    def itercol_categorical(column):
        nonlocal column_idx
        col_name = columns_delta_table[column_idx]
        delta_table_column = delta_table_multifunctional[col_name].replace(1, 200)
        delta_table_column = delta_table_column.replace(-1, -200)
        s = np.array(list(zip(delta_table_column.abs(),
                              delta_table[columns_delta_table[0]].abs())),
                     dtype=[('value', delta_table_column),
                            ('tiebreaker', delta_table.dtypes[columns_delta_table[0]])])
        sorted_idx = np.argsort(s, order=['value', 'tiebreaker'])
        num_ones = (delta_table_multifunctional[col_name] == 1).sum() + (delta_table_multifunctional[col_name] == -1).sum()
        new_stop_line.at[col_name] = len(delta_table_multifunctional) - num_ones
        column_idx += 1
        return sorted_idx
    sorted_table[categorical_att_columns] = sorted_table[categorical_att_columns].apply(itercol_categorical, axis=0)
