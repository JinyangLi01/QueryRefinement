...
def update_minimal_refinement(minimal_added_relaxations, r, this_refinement, minimal_refinements):
    dominated = []
    for mr in minimal_added_relaxations:
        if mr == r:
            return minimal_added_relaxations, minimal_refinements
        if dominate(mr, r):
            return minimal_added_relaxations, minimal_refinements
        elif dominate(r, mr):
            dominated.append(mr)
    if len(dominated) > 0:
        idx = [i for i in range(len(minimal_refinements)) if minimal_added_relaxations[i] not in dominated]
        minimal_added_relaxations = minimal_added_relaxations[idx]
        minimal_refinements = minimal_refinements[idx]
    minimal_added_relaxations.append(r)
    minimal_refinements.append(copy.deepcopy(this_refinement))
    return minimal_added_relaxations, minimal_refinements
...
def LatticeTraversal(data, selected_attributes, sensitive_attributes, fairness_constraints,
    ...
    if only_greater_than:
        return LatticeTraversalRelaxation(data, selected_attributes, sensitive_attributes, fairness_constraints,
                                          numeric_attributes, categorical_attributes, selection_numeric_attributes,
                                          selection_categorical_attributes, time_limit=5 * 60)
    elif only_smaller_than:
        return LatticeTraversalContraction(data, selected_attributes, sensitive_attributes, fairness_constraints,
                                           numeric_attributes, categorical_attributes, selection_numeric_attributes,
                                           selection_categorical_attributes, time_limit=5 * 60)
    else:
        ...
        if att_idx == num_numeric_att + num_cate_variables_to_add + num_cate_variables_to_remove:
            if whether_satisfy_fairness_constraints(data, selected_attributes, sensitive_attributes,
                ...
                minimal_refinement_values = transform_refinement_format(this_refinement, numeric_att_domain_to_relax,
                                                                        ...
                                                                        numeric_attributes)
                minimal_added_relaxations, minimal_refinements = update_minimal_refinement(minimal_added_relaxations,
                                                                                          minimal_refinement_values,
                                                                                          this_refinement,
                                                                                          minimal_refinements)
            att_idx -= 1
...
def LatticeTraversalContraction(data, selected_attributes, sensitive_attributes, fairness_constraints,
                                numeric_attributes, categorical_attributes, selection_numeric_attributes,
                                selection_categorical_attributes, time_limit=5 * 60):
    # TODO
...
def LatticeTraversalRelaxation(data, selected_attributes, sensitive_attributes, fairness_constraints,
                               numeric_attributes, categorical_attributes, selection_numeric_attributes,
                               selection_categorical_attributes, time_limit=5 * 60):
...
def FindMinimalRefinement(data_file, query_file, constraint_file):
    ...
    time1 = time.time()
    minimal_refinements, numeric_att_domain_to_relax, categorical_att_domain_to_add, \
